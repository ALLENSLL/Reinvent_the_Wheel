...
function CreateTable(){
	...
	if (ProcessSum != "1" && ProcessSum != "2" && ProcessSum != "3" && ProcessSum != "4" && ProcessSum != "5" && ProcessSum != "6" && ProcessSum != "7" && ProcessSum != "8" && ProcessSum != "9" && ProcessSum != "10") {
		alert("提示！\n进程数：1~10");
	}//判断输入是否合法
	else {
...
//FCFS函数（主函数）由Initiailization()调用
function fcfs() {
	Createlist();
	while (ReadyList.length!=0)
	{
		var temp=ReadyList.shift();
		while (ProcessList[temp-1].cometime>NowTime)
		{
			showprocess(0);
			NowTime+=1;
		}
		for (var i=0; i<ProcessList[temp-1].staytime; i++)
		{
			showprocess(temp);
			NowTime+=1; //执行完一个单位时间
			doprocess(temp);
...
//创建就绪队列，存为ReadyList，由fcfs()调用
function Createlist() {
	var templist=new Array();
	var temp=0;
	for (var i=0; i<ProcessSum; i++)
	{
		templist[i]=ProcessList[i].processnum;
	}
	for (var i=0; i<ProcessSum; i++)
	{
		for (var j=0; j<ProcessSum-i-1; j++)
		{
			if (ProcessList[templist[j]-1].cometime>ProcessList[templist[j+1]-1].cometime)
			{
				temp=templist[j];
	...
	for (var i=0; i<ProcessSum; i++)
	{
		ReadyList.push(templist[i]);//加入就绪队列 .shift()出队列
	}
}
//绘制单位时间块
