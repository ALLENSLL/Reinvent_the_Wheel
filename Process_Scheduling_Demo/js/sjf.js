...
function selectprocess() {
	...
	for (var i=0; i<ProcessSum; i++)
	{
		if ((ProcessList[i].cometime<=NowTime) && (ProcessList[i].laststaytime!=0))
		{
			templist.push(i+1);
		}
	}
	if (templist.length==0)
	{
		...
		{
			if (ProcessList[templist[j]-1].staytime<ProcessList[temp-1].staytime)
			{
				temp=templist[j];
			}
		}
